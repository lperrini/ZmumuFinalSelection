...
##### creates directory and file list for job #######
       ...
       dirScript = "tmp_tt_embedding/"+sample_info["nameout"][i]+"/"
       fout = open("tmp_tt_embedding/"+sample_info["nameout"][i]+"/resubmit.sh", 'w')
       count_found  = 0
       count_not_found  = 0
       count_small_size = 0
       count = 0
       for item in os.listdir(dirScript):
           ...
           if glob.glob("../data/ntuples/tt_embedding/"+sample_info["nameout"][i]+"_"+fileNumber[1]+".root"):
              statinfo = os.stat("../data/ntuples/tt_embedding/"+sample_info["nameout"][i]+"_"+fileNumber[1]+".root")
              fileSize = statinfo.st_size
              if(fileSize < 3000):
                 fout.write("bsub -q 1nw -o logs job"+fileNumber[1]+".sh \n")
                 count_small_size=count_small_size+1
              else:
       ...
       count = count_not_found+count_small_size
      
       if(njobs==count_found): 
           print "#### DONE ### --> SAMPLE: %s ; NAMEOUT: %s; " %(sample_name,sample_info["nameout"][i])
       else: 
           print "SAMPLE: %s ; NAMEOUT: %s; NJOBS: %s; FILES FOUND: %s; STILL to Run: %s" %(sample_name,sample_info["nameout"][i],str(njobs),str(count_found),str(count))
