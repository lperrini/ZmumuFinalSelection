...
for sample_name, sample_info in samples.items():
   ...
   ##### creates directory and file list for job #######
      ...
      for ijobs in range(0,njobs):
         ...
         fout.write("cmsenv\n")
         fout.write("cd "+str(path)+"\n")
         fout.write(ScriptName+ " inputFile=/store/user/veelken/Lucia/preselected_V2/"+sample_name+".root outputFile="+outDir+"/"+sample_info["nameout"][i]+"_"+str(ijobs)+".root xsection="+str(n_xsec)+" nevents="+str(n_events)+" sample="+sample_name+" label="+sample_info["labels"][i]+" channel=mm embedding=yes channelEB=tt ptcut1=40.0 etacut1=2.1 ptcut2=40.0 etacut2=2.1 met=yes dataOverMadgraphRW=no nstart="+str(nstart)+" nstop="+str(nstop)+" \n")
